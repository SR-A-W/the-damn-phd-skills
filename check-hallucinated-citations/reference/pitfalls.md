...
### A6b. ⭐ 你自己的**去连字符**会破坏合法连字符（PDF 路径专属，极易误报）

PDF 里换行断词写作 `xx-\nyy`，提取后要接回成 `xxyy`。但如果原文**本来就带连字符**且恰好在那里换行，
接回就把合法连字符删掉了。

真实案例：书目里的 `https://novasky-` / `ai.github.io/...`（在连字符处换行）被我的清洗函数变成
`novaskyai.github.io` → 实测 404 → 差点报成"死链"。**回查未清洗的原始栏文本才发现 PDF 里是对的。**

**纪律**：凡是要报 URL / 带连字符的专名（`novasky-ai`、`gpt-oss`、`o1-like`、`Chain-of-Thought`）有问题，
**必须回查未经去连字符处理的原始提取文本**再下结论。去连字符只用于生成给 agent 看的可读串，
不作为字段错误的判据。

### A6c. 双栏 PDF 必须分栏提取

`pdftotext -layout` 会把左右两栏的文字**交叉混在同一行**，得到的书目是乱的。
- 用默认模式（无 `-layout`）能拿到正确阅读序，但**丢掉悬挂缩进**（不好切条目）；
- 最稳：**按栏裁剪 + `-layout`**（`pdftotext -layout -x <左边界> -W <栏宽>`），既有正确栏序又保留缩进。
- 切条目按缩进：**只有缩进 0 才是新条目**（续行缩进可能是 1 也可能是 2，阈值写成 `<=1` 会切碎长作者列表）。
- 兜底：ACL 风格每条必含 `. YYYY[a-z]. `；不含年份标记的片段一律并入上一条。
...
### B4b. 纯 PDF 做维度 7 时，**别忘了书目之后的附录**

正文里的引用上下文不止在书目之前。真实案例：一篇 ACL 投稿的 63 条引用里，
只截取"References 之前"的正文只能匹配到 42 条；把**书目之后的附录**（第 14–21 页）也算进去后
才达到 63/63。**自引尤其集中在附录**（相关工作/broader impact 一类）。
→ 上下文语料 = `References 之前的正文` + `附录`，中间的书目段本身要排除。
